...
class DatabaseManager:
    ...
    def _init_db(self):
        with self.get_cursor() as conn:
            ...
            try:
                # Performance Optimization: Add composite indexes on (embedding_id, tenant, project_id)
                # to prevent O(N) full table scans when resolving vector search results via `get_memories_by_embedding_ids`.
                conn.execute("""
