...
class VectorStore:
    ...
    def __init__(self, index_path: str, dimension: int):
        ...
        # ⚡ Bolt: Use a list buffer for amortized O(1) batch insertions instead of O(N^2) np.vstack
    ...
    def _merge_buffers(self):
        """Merge buffered vectors into the main arrays."""
        if not self._unmerged_vectors:
            return

        new_vectors = np.vstack(self._unmerged_vectors)
        new_ids = np.concatenate(self._unmerged_ids)
        new_norms = np.concatenate(self._unmerged_norms)

        self.vectors = np.vstack([self.vectors, new_vectors])
        self.ids = np.concatenate([self.ids, new_ids])
        self.norms = np.concatenate([self.norms, new_norms])

        self._unmerged_vectors.clear()
        self._unmerged_ids.clear()
        self._unmerged_norms.clear()
