...
class VectorStore:
    ...
    def ntotal(self):
        with self.lock:
            buffer_len = sum(len(b) for b in self._id_buffer)
            return len(self.ids) + buffer_len


    def _flush_buffers(self):
        """Merges buffered additions into the main arrays. Assumes lock is held."""
    ...
    def add_vectors(self, vectors: np.ndarray, ids: List[int]):
        ...
        with self.lock:
            # ⚡ Bolt Optimization: Use amortized O(1) list buffering instead of O(N) np.vstack
