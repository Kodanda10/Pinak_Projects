...
class VectorStore:
    ...
    def __init__(self, index_path: str, dimension: int):
        ...
        # ⚡ Bolt: Buffers to prevent O(N^2) array copies on every insertion
    ...
    def ntotal(self):
        with self.lock:
            return len(self.ids) + sum(len(ids) for ids in self._ids_buffer)

    def _flush_buffers(self):
        # ⚡ Bolt: Consolidate buffers into arrays to amortize insertion costs
        # Caller must hold self.lock
        if not self._ids_buffer:
            return
    ...
    def add_vectors(self, vectors: np.ndarray, ids: List[int]):
        ...
        with self.lock:
            # ⚡ Bolt: Append to list buffers instead of costly np.vstack
    ...
    def search(self, query_vector: np.ndarray, k: int = 10) -> Tuple[List[float], List[int]]:
        ...
        with self.lock:
            self._flush_buffers()
            if len(self.ids) == 0 and len(self._ids_buffer) == 0:
                return [], []
