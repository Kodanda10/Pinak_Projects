...
class VectorStore:
    ...
    def __init__(self, index_path: str, dimension: int):
        ...
        # ⚡ Bolt: Add O(1) append buffers to prevent O(N) np.vstack bottlenecks
        self._unflushed_vectors = []
        self._unflushed_ids = []
        self._unflushed_norms = []
    ...
    def ntotal(self):
        with self.lock:
            return len(self.ids) + sum(len(ids) for ids in self._unflushed_ids)

    def _flush_buffers(self):
        """Flush buffered vectors into the main numpy arrays. Assumes lock is held."""
        if not self._unflushed_ids:
            return

        new_vectors = np.vstack(self._unflushed_vectors)
        new_ids = np.concatenate(self._unflushed_ids)
        new_norms = np.concatenate(self._unflushed_norms)
    ...
    def add_vectors(self, vectors: np.ndarray, ids: List[int]):
        ...
        with self.lock:
            # ⚡ Bolt: Use O(1) list append instead of O(N) np.vstack to prevent
            # performance bottleneck on large indexes during repeated additions
            self._unflushed_vectors.append(vectors)
            self._unflushed_ids.append(id_array)
            self._unflushed_norms.append(new_norms)
