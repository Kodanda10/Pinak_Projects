...
class VectorStore:
    ...
    Implements dynamic array resizing for O(1) amortized insertion.
    """
    def __init__(self, index_path: str, dimension: int):
        ...
        # In-memory storage with capacity tracking
        self.size = 0
            ...
            if load_path:
                try:
                    ...
                    if self.capacity < count:
                        self.capacity = max(count, self.capacity * 2)

                    # Re-allocate if needed or just use loaded data if we want to be simple
                    # But we want to maintain the capacity buffer.
                    if self.vectors.shape[0] != self.capacity:
    ...
    def _resize(self, min_capacity: int):
        ...
        logger.debug(f"Resized Vector Store to capacity {self.capacity}")
    ...
    def remove_ids(self, ids: List[int]):
        ...
        with self.lock:
            ...
            if kept_count < self.size:
                ...
                # Note: boolean indexing returns a copy, so we assign it back to the slice
                self.vectors[:kept_count] = self.vectors[:self.size][mask]
                self.ids[:kept_count] = self.ids[:self.size][mask]
                self.norms[:kept_count] = self.norms[:self.size][mask]
    ...
    def reset(self):
        with self.lock:
            self.size = 0
            # Optional: Shrink capacity if too large?
            # For now, keep capacity to avoid reallocation churn if it fills up again.
            self.needs_save = True
