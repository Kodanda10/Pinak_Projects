...
class VectorStore:
    ...
    def __init__(self, index_path: str, dimension: int):
        ...
        # ⚡ Bolt: Buffers for O(1) amortized insertions
    ...
    def _flush_buffers(self):
        """Flushes internal buffers to main arrays. Caller must hold lock."""
        if not self._vectors_buffer:
            return

        # ⚡ Bolt: Single flush step avoids O(N^2) overhead of sequential vstacks
    ...
    def ntotal(self):
        with self.lock:
            # ⚡ Bolt: O(1) total computation by summing batch sizes instead of flushing
    ...
    def add_vectors(self, vectors: np.ndarray, ids: List[int]):
        ...
        with self.lock:
            # ⚡ Bolt: Append to buffer instead of O(N) vstack for amortized O(1) insertions
