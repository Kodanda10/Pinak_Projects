...
class VectorStore:
    ...
    def __init__(self, index_path: str, dimension: int):
        ...
        # O(1) buffer for vector additions to avoid np.vstack bottleneck
        self._vector_buffer = []
        self._id_buffer = []
        self._norm_buffer = []
    ...
    def ntotal(self):
        with self.lock:
            buffer_len = sum(len(ids) for ids in self._id_buffer)
            return len(self.ids) + buffer_len


    def _flush_buffers(self):
        """Flush buffered additions to the main arrays. Assumes lock is held."""
        if not self._vector_buffer:
            return

        new_vectors = np.vstack(self._vector_buffer)
        new_ids = np.concatenate(self._id_buffer)
        new_norms = np.concatenate(self._norm_buffer)
    ...
    def add_vectors(self, vectors: np.ndarray, ids: List[int]):
        ...
        with self.lock:
            # Buffer additions to avoid O(N) np.vstack overhead per call
            self._vector_buffer.append(vectors)
            self._id_buffer.append(id_array)
            self._norm_buffer.append(new_norms)
