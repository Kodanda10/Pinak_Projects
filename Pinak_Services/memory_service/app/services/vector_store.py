...
class VectorStore:
    ...
    def __init__(self, index_path: str, dimension: int, initial_capacity: int = 1024):
        ...
        # In-memory storage with dynamic capacity
    ...
    def _resize(self, new_capacity: int):
        """Internal helper to resize the pre-allocated buffers."""
    ...
    def search(self, query_vector: np.ndarray, k: int = 10) -> Tuple[List[float], List[int]]:
        ...
        with self.lock:
            ...
            # Operating on valid slice
            valid_vectors = self.vectors[:self.size]
            valid_norms = self.norms[:self.size]
