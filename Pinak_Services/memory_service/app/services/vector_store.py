...
class VectorStore:
    ...
    def _flush_buffers(self):
        """Flush the list buffers into the main numpy arrays."""
        if not self._vectors_buffer:
            return

        new_vectors = np.vstack(self._vectors_buffer)
        new_ids = np.concatenate(self._ids_buffer)
        new_norms = np.concatenate(self._norms_buffer)

        self.vectors = np.vstack([self.vectors, new_vectors]) if self.vectors.size else new_vectors
        self.ids = np.concatenate([self.ids, new_ids]) if self.ids.size else new_ids
        self.norms = np.concatenate([self.norms, new_norms]) if self.norms.size else new_norms
