...
class VectorStore:
    ...
    def __init__(self, index_path: str, dimension: int):
        ...
        # Buffers for O(1) addition
        self._vector_buffers = []
        self._id_buffers = []
        self._norm_buffers = []
    ...
    def _flush_buffers(self):
        """Flushes buffers into main arrays. Assumes lock is acquired."""
        if self._id_buffers:
            self.vectors = np.vstack([self.vectors] + self._vector_buffers)
            self.ids = np.concatenate([self.ids] + self._id_buffers)
            self.norms = np.concatenate([self.norms] + self._norm_buffers)
            self._vector_buffers.clear()
            self._id_buffers.clear()
    ...
    def ntotal(self):
        with self.lock:
            return len(self.ids) + sum(len(arr) for arr in self._id_buffers)
    ...
    def add_vectors(self, vectors: np.ndarray, ids: List[int]):
        ...
        with self.lock:
            # Buffer additions instead of O(N) vstacking
            self._vector_buffers.append(vectors)
            self._id_buffers.append(id_array)
            self._norm_buffers.append(new_norms)
