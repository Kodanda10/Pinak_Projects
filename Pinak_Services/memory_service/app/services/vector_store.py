...
class VectorStore:
    ...
    def ntotal(self):
        with self.lock:
            return len(self.ids) + sum(len(b) for b in self._buffer_ids)

    def _flush_buffers(self):
        """Flushes the O(1) buffers to the O(N) main arrays."""
        if self._buffer_ids:
            self.vectors = np.vstack([self.vectors] + self._buffer_vectors)
            self.ids = np.concatenate([self.ids] + self._buffer_ids)
            self.norms = np.concatenate([self.norms] + self._buffer_norms)
            self._buffer_vectors.clear()
            self._buffer_ids.clear()
            self._buffer_norms.clear()
    ...
    def search(self, query_vector: np.ndarray, k: int = 10) -> Tuple[List[float], List[int]]:
        ...
        with self.lock:
            self._flush_buffers()
            if len(self.ids) == 0 and not self._buffer_ids:
                return [], []
