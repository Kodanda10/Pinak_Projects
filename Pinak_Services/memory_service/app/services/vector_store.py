...
class VectorStore:
    ...
    def ntotal(self):
        with self.lock:
            return len(self.ids) + sum(len(b) for b in self._id_buffer)
    ...
    def _flush_buffers(self):
        """Merge buffered additions into the main arrays to amortize O(N) concatenation costs."""
        if self._vector_buffer:
            self.vectors = np.vstack([self.vectors] + self._vector_buffer)
            self.ids = np.concatenate([self.ids] + self._id_buffer)
            self.norms = np.concatenate([self.norms] + self._norm_buffer)
    ...
    def add_vectors(self, vectors: np.ndarray, ids: List[int]):
        ...
        with self.lock:
            # ⚡ Bolt: Amortized O(1) list-based buffering avoids O(N) np.vstack bottleneck on every insert
