...
class VectorStore:
    ...
    def __init__(self, index_path: str, dimension: int):
        ...
        # In-memory storage with dynamic sizing
        self._size = 0
        self._capacity = 1000  # Initial capacity
        ...
        with self.lock:
            ...
            if load_path:
                try:
                    ...
                    # Ensure capacity
                    if loaded_size > self._capacity:
                        self._capacity = max(loaded_size * 2, 1000)
        ...
        with self.lock:
            if self.needs_save:
                ...
                with open(self.index_path, "wb") as handle:
                    # Save only the active portion of the arrays
                    np.save(handle, {
    ...
    def add_vectors(self, vectors: np.ndarray, ids: List[int]):
        ...
        new_vector_norms = np.sum(np.square(vectors), axis=1)

        with self.lock:
            # Check capacity and resize if necessary
            if self._size + n_new > self._capacity:
                new_capacity = max(self._capacity * 2, self._size + n_new + 1000)

                new_vectors_buf = np.empty((new_capacity, self.dimension), dtype=np.float32)
                new_ids_buf = np.empty(new_capacity, dtype=np.int64)
                new_norms_buf = np.empty(new_capacity, dtype=np.float32)

                # Copy existing data
                new_vectors_buf[:self._size] = self.vectors[:self._size]
                new_ids_buf[:self._size] = self.ids[:self._size]
                new_norms_buf[:self._size] = self.norms[:self._size]

                self.vectors = new_vectors_buf
                self.ids = new_ids_buf
                self.norms = new_norms_buf
                self._capacity = new_capacity

            # Add new data
            self.vectors[self._size : self._size + n_new] = vectors
            self.ids[self._size : self._size + n_new] = id_array
            self.norms[self._size : self._size + n_new] = new_vector_norms
    ...
    def remove_ids(self, ids: List[int]):
        ...
        with self.lock:
            ...
            # If no items removed, do nothing
