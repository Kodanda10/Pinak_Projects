...
class VectorStore:
        ...
        with self.lock:
            ...
            if load_path:
                try:
                    ...
                    logger.info(
                        f"Loaded Vector Store from {load_path}. "
                        f"Size: {len(self.ids)}"
                    )
    ...
    def save(self):
        ...
        with self.lock:
            if self.needs_save:
                ...
                self.needs_save = False
                logger.info(
                    f"Saved Vector Store to {self.index_path}. "
                    f"Size: {len(self.ids)}"
                )
    ...
    def add_vectors(self, vectors: np.ndarray, ids: List[int]):
        ...
        if vectors.shape[1] != self.dimension:
            raise ValueError(
                f"Vector dimension {vectors.shape[1]} does not "
        ...
        state = getattr(self, '_local', None)
        if state is None:
            state = threading.local()
            self._local = state

        if not hasattr(state, 'batch_depth'):
            state.batch_depth = 0
            state.batch_vectors = []
            state.batch_ids = []
            state.batch_norms = []
        ...
        if state.batch_depth > 0:
            state.batch_vectors.append(vectors)
            state.batch_ids.append(id_array)
            state.batch_norms.append(new_norms)
        else:
            with self.lock:
    ...
    def batch_add(self):
        ...
        try:
            ...
            # Only commit on the outermost context manager exiting normally
            if state.batch_depth == 1 and state.batch_vectors:
                with self.lock:
                    self.vectors = np.vstack(
                        [self.vectors] + state.batch_vectors
                    )
                    self.ids = np.concatenate([self.ids] + state.batch_ids)
                    self.norms = np.concatenate(
                        [self.norms] + state.batch_norms
                    )
                    self.needs_save = True
                self.save()
        finally:
