...
class MemoryService:
    ...
    def update_memory(self, layer: str, memory_id: str, updates: Dict[str, Any], tenant: str, project_id: str) -> bool:
        ...
        """
        # Security: Allow only specific fields per layer (Mass Assignment Protection)
        ALLOWED_UPDATES = {
        ...
        if not safe_updates:
            logger.warning(f"Update rejected: No allowed keys found for layer {layer}. Input: {updates.keys()}")
            return False
