...
def mint(tenant: str, project: str = "default", secret: str = None):
    ...
    # 🛡️ Sentinel: Removed hardcoded fallback secret to enforce secure configuration.
    jwt_secret = secret or os.environ.get("PINAK_JWT_SECRET")
    if not jwt_secret:
        typer.secho("Error: PINAK_JWT_SECRET environment variable is not set.", fg=typer.colors.RED, err=True)
