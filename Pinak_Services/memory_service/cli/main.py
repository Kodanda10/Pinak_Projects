...
def mint(tenant: str, project: str = "default", secret: str = None):
    ...
    # 🛡️ Sentinel: Removed hardcoded default JWT secret. Fail securely if no secret is provided.
    if not jwt_secret:
        typer.echo("Error: PINAK_JWT_SECRET environment variable or --secret argument must be set", err=True)
...
def search(query: str, tenant: str = "demo", project: str = "default", url: str = "http://localhost:8001"):
    ...
    # 🛡️ Sentinel: Removed hardcoded default JWT secret. Fail securely if no secret is provided.
    if not token_secret:
        typer.echo("Error: PINAK_JWT_SECRET environment variable must be set", err=True)
        raise typer.Exit(code=1)
    token_payload = {
        "sub": "search-cli",
        "tenant": tenant,
