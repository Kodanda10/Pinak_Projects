...
def mint(tenant: str, project: str = "default", secret: str = None):
    ...
    # 🛡️ Sentinel: Removed hardcoded JWT secret to prevent token forgery vulnerabilities.
    jwt_secret = secret or os.environ.get("PINAK_JWT_SECRET")
    if not jwt_secret:
        typer.echo("Error: PINAK_JWT_SECRET environment variable or --secret argument is required.")
...
def search(query: str, tenant: str = "demo", project: str = "default", url: str = "http://localhost:8001"):
    ...
    if not token_secret:
        typer.echo("Error: PINAK_JWT_SECRET environment variable is required.")
        raise typer.Exit(code=1)
