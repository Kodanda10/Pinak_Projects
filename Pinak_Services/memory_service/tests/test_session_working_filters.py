...
async def test_session_add_ttl_and_list_paging():
    async with LifespanManager(app):
        async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as ac:
            ...
            # Insert 3 items
            for i in range(3):
                r = await ac.post('/api/v1/memory/session/add', json={'session_id': sid, 'content': f'c{i}'})
                assert r.status_code==201
            # Page 2 items
            lst = await ac.get('/api/v1/memory/session/list', params={'session_id': sid, 'limit': 2, 'offset': 1})
...
async def test_working_ttl_respected():
    async with LifespanManager(app):
        async with AsyncClient(transport=ASGITransport(app=app), base_url='http://test') as ac:
            ...
            r1 = await ac.post('/api/v1/memory/working/add', json={'content':'old','expires_at': past})
            assert r1.status_code==201
            # valid record
            r2 = await ac.post('/api/v1/memory/working/add', json={'content':'new','ttl_seconds': 100})
