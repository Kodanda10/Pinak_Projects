...
from app.services.memory_service import (
    add_episodic, list_episodic,
    add_procedural, list_procedural,
    add_rag, list_rag,
    search_v2
)
import asyncio
...
class TestMemoryIntegration:
    ...
    def test_cross_layer_memory_operations(self, memory_service):
        ...
        episodic_result = add_episodic(memory_service, tenant, project_id, episodic_data['content'], episodic_data.get('metadata', {}).get('importance', 5))
        episodic_data['result'] = episodic_result

        # Add to procedural layer
        procedural_result = add_procedural(memory_service, tenant, project_id, procedural_data['content'], [procedural_data.get('metadata', {}).get('category', 'general')])
        procedural_data['result'] = procedural_result

        # Add to RAG layer
        rag_result = add_rag(memory_service, tenant, project_id, rag_data['content'], rag_data.get('metadata', {}).get('domain', 'general'))
        rag_data['result'] = rag_result

        # Verify all layers have data
        episodic_path = memory_service._dated_file(memory_service._store_dir(tenant, project_id), 'episodic', 'episodic')
        procedural_path = memory_service._dated_file(memory_service._store_dir(tenant, project_id), 'procedural', 'procedural')
        rag_path = memory_service._dated_file(memory_service._store_dir(tenant, project_id), 'rag', 'rag')
    ...
    def test_memory_search_across_layers(self, memory_service):
        ...
        for layer, content in test_data:
            if layer == 'episodic':
                ...
                add_procedural(memory_service, tenant, project_id, f'skill_{len(test_data)}_{test_id}', [content])
            elif layer == 'rag':
                add_rag(memory_service, tenant, project_id, content, f'test_source_{test_id}')

        # Test search across all layers
        search_results = search_v2(memory_service, tenant, project_id, 'Python', ['episodic', 'procedural', 'rag'])
        ...
        python_results = [r for r in all_results if 'Python' in r.get('content', '') or 'Python' in r.get('query', '') or 'Python' in str(r.get('steps', []))]
    ...
    def test_audit_trail_integration(self, memory_service):
        ...
        add_procedural(memory_service, tenant, project_id, f'test_skill_{test_id}', [f'step1_{test_id}', f'step2_{test_id}'])
        add_rag(memory_service, tenant, project_id, f'Test RAG {test_id}', f'test_source_{test_id}')

        # Search to trigger search audit
        search_v2(memory_service, tenant, project_id, test_id, ['episodic', 'procedural', 'rag'])
        ...
        if os.path.exists(audit_path):
            ...
            search_operations = [entry for entry in audit_data if 'search' in entry.get('operation', '')]
