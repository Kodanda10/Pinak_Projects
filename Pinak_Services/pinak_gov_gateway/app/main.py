...
REQUIRE_PROJECT_HEADER = os.getenv("REQUIRE_PROJECT_HEADER", "true").lower() in {
    "1",
    "true",
    "yes",
    "on",
...
ALLOWED_ROLES = {
    r.strip()
    for r in os.getenv("PINAK_ALLOWED_ROLES", "viewer,editor,admin").split(",")
    if r.strip()
}
...
async def proxy(
    ...
    async with httpx.AsyncClient(
        follow_redirects=True, timeout=20.0, verify=verify_param
    ) as client:
        ...
        if OPA_URL:
            try:
                ...
                r = await client.post(
                    f"{OPA_URL.rstrip('/')}/v1/data/pinak/policy/allow",
                    json={"input": opa_in},
                    timeout=5.0,
                )
    ...
    try:
        if (
            ...
            try:
                ...
                cert_opt = (
                    (MEMORY_API_CLIENT_CERT, MEMORY_API_CLIENT_KEY)
                    if MEMORY_API_CLIENT_CERT and MEMORY_API_CLIENT_KEY
                    else None
                )
                async with httpx.AsyncClient(
                    timeout=5.0, verify=verify_mem, cert=cert_opt
                ) as client:
                    await client.post(
                        f"{MEMORY_API_URL}/api/v1/memory/event",
                        json={
                            "type": "gov_audit",
                            "path": f"/{path}",
                            "method": method,
