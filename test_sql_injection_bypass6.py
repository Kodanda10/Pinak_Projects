...
# The problem is that sqlite3.execute only executes the FIRST statement.
# If we inject a completely valid expression inside the SET clause without semicolon...
updates = {"secret": "hacked", "secret = (SELECT 'hacked' FROM test WHERE id = '1')": "value"}
