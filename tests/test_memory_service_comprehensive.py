...
def test_memory_data():
    ...
    return {
        ...
        "working": {
            "content": "Immediate task: implement TDD framework",
            "ttl": 1800
        },
        "events": {
            "type": "user_action",
    ...
    async def test_search_all_layers(self, memory_manager):
        ...
        with patch.object(memory_manager.client, 'get') as mock_get:
            ...
            mock_response.json.return_value = {
                "data": {
                    "episodic": [
                        {"id": "epi_1", "content": "Python development experience", "relevance": 0.9}
                    ],
                    "procedural": [
                        {"id": "proc_1", "skill_id": "python_debug", "relevance": 0.7}
                    ],
                    "rag": [
                        {"id": "rag_1", "query": "Python best practices", "relevance": 0.8}
                    ]
                }
            }
            mock_get.return_value = mock_response
