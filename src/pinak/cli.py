        ...
        try:
            ...
            run(
                [
                    ...
                    "compose",
                    "-f",
                    str(base / "infra" / "docker-compose.yml"),
                    "up",
                    "-d",
                ],
                check=True,
    ...
    if not ensure_docker():
        ...
        try:
            ...
            bargs = [
                ...
                "--url",
                args.url or os.getenv("PINAK_MEMORY_URL", "http://localhost:8011"),
            ]
            if args.tenant:
                bargs += ["--tenant", args.tenant]
            if args.token:
                bargs += ["--token", args.token]
            bridge_main(bargs)
        except SystemExit:
            pass
        except Exception as e:
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    if not (Path.cwd() / ".pinak" / "pinak.json").exists():
        ok = False
        print("No project identity found (.pinak/pinak.json)")
    # ensure .pinak is not tracked by git
    if (Path.cwd() / ".git").exists():
        try:
            out = subprocess.run(
                ["git", "ls-files", "--error-unmatch", ".pinak"], capture_output=True
            )
            if out.returncode == 0:
                ok = False
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    t.add_argument("--exp", type=int, default=None, help="Expiry in minutes (optional)")
    ...
    quarantine = sub.add_parser(
        "quarantine", help="File quarantine system for safe file management"
    )
    quarantine.add_argument("rest", nargs=argparse.REMAINDER)
    quarantine.set_defaults(func=cmd_quarantine)

    # one-click orchestration
    up = sub.add_parser("up", help="Start Memory + Gov + Parlant (compose)")
    up.set_defaults(func=cmd_up)
    dn = sub.add_parser("down", help="Stop services (compose)")
