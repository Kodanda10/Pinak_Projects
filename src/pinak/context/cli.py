...
class DemoStore(IContextStore):
    ...
    async def retrieve(self, query):
        from pinak.context.core.models import ContextResponse, ContextItem
        from datetime import datetime, timezone
        from pinak.context.core.models import ContextLayer, ContextPriority, SecurityClassification
    ...
    async def search_similar(self, content, limit=10):
        ...
        ] if "build" in content.lower() else []
...
async def async_get_context(topic: str) -> Dict[str, Any]:
    ...
    try:
        ...
        from pinak.context.core.models import ContextQuery, ContextLayer, SecurityClassification
...
def redact_sensitive_content(text: str) -> str:
    ...
    sensitive_patterns = [
        (r'\b\d{4}[\s\-]\d{4}[\s\-]\d{4}[\s\-]\d{4}\b', '[REDACTED CREDIT CARD]'),  # Credit cards
...
def apply_redaction_to_items(items: List[Dict[str, Any]], redact: bool = True) -> List[Dict[str, Any]]:
...
def main():
    ...
    if args.cmd == "now":
        ...
        if args.json:
            ...
            items = ctx.get("items", [])
            redacted_items = apply_redaction_to_items(items, redact=not getattr(args, 'no_redact', False))
