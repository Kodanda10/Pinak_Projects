...
from .file_quarantine import (
    get_quarantine_manager, QuarantineAction, QuarantinePriority,
    safe_delete, safe_move
)
...
def main(argv: List[str] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="pinak-quarantine",
        description="File Quarantine System CLI for Pinak"
    )
    ...
    quarantine_parser = subparsers.add_parser(
        'quarantine', help='Quarantine a file'
    )
    quarantine_parser.add_argument('file', type=Path, help='File to quarantine')
    quarantine_parser.add_argument(
        '--action', choices=[a.value for a in QuarantineAction],
    ...
    quarantine_parser.add_argument(
        '--user', default='cli_user',
        help='User performing the action'
    )
    quarantine_parser.add_argument(
        '--priority', choices=[p.value for p in QuarantinePriority],
    ...
    restore_parser = subparsers.add_parser(
        'restore', help='Restore a quarantined file'
    )
    ...
    list_parser = subparsers.add_parser(
        'list', help='List quarantined files'
    )
    ...
    stats_parser = subparsers.add_parser(
        'stats', help='Show quarantine statistics'
    )
    ...
    safe_delete_parser.add_argument(
        '--user', help='User performing the action'
    )
    safe_delete_parser.add_argument(
        '--no-patch', action='store_true',
        help='Do not patch system file operations'
    )
    ...
    safe_move_parser.add_argument(
        '--reason', required=True,
        help='Reason for move'
    )
    safe_move_parser.add_argument(
        '--user', help='User performing the action'
    )
