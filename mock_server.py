...
async def add_procedural(item: dict):
    item = dict(item)
    item["id"] = f"p{len(DB['procedural'])+1}"
    item["ts"] = _ts()
...
async def add_working(item: dict):
    key = item.get("key")
    exp_in = int(item.get("expires_in", 600))
    value = item.get("value")
    if not key:
        raise HTTPException(400, "key required")
...
def search_all_layers(q: str):
    ...
    for it in DB["episodic"]:
        if ql in str(it).lower():
            matches["episodic"].append(it)
    for it in DB["procedural"]:
        if ql in str(it).lower():
            matches["procedural"].append(it)
    for it in DB["rag"]:
        if ql in str(it).lower():
            matches["rag"].append(it)
